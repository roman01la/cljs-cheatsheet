...
(hiccups/defhtml basics-tooltips []
  ...
  [:div#tooltip-define.tooltip-53dde {:style "display:none"}
    [:p "В ClojureScript все данные неизменяемы по умолчанию, что означает, что "
        "значение симовала не может быть изменено после того, как он был определен."]]
  ...
  [:div#tooltip-branch.tooltip-53dde {:style "display:none"}
    ...
    [:p "Это более простое поведение, чем в JavaScript, в котором правила приведения типов "
        "намного сложнее."]
    (truthy-table)]
  ...
  [:div#tooltip-atoms.tooltip-53dde {:style "display:none"}
    ...
    [:p
      ...
      "Это означает, что вы не можете изменить значение чего-то после того, "
      "как оно было определено."]
    [:p
      "Атомы делают возможным изменяние значения. Так же они разделяют то, "
      "как значение изменяется и считывается, что делает состояние более понятным."]
    [:p
      "Наблюдающие функции выполяються, когда значение изменяется."
      "Это идеальный поведение, когда данные являются отображением UI."]]

  [:div#tooltip-functions.tooltip-53dde {:style "display:none"}
    [:p
      "Функции в ClojureScript такие же, как и в JavaScript, "
      "и они могут быть использованы точно так же, как и функции в JavaScript."]
    [:p
      "Базовая библиотека имеет множество полезных функций высшего порядка. "
      "Так же в ClojureScript есть удобное сокращение для создания анонимных функций."]]
...
(hiccups/defhtml collections-tooltips []
  [:div#tooltip-collections.tooltip-53dde {:style "display:none"}
    ...
    [:p
      ...
      "операцией."]
    [:p
      "У всех коллекции есть представление в виде литерала:"]
    [:table.tbl-3160a
      [:thead
        [:tr
